...
## Ursprünglicher Playbook-Aufbau
...
## Neue Playbook-Struktur

Ziel ist ein einziges Playbook — `pb_setup_app_deploy.yml` — das alle Anwendungs-Deployments unabhängig von Typ und Zielumgebung abwickelt.

Statt für jede App ein eigenes Playbook zu schreiben (Tasks gibt es natürlich trotzdem), wird das Deployment vollständig über ein **AWX-Credential** gesteuert. Man wählt die App, den Deployment-Typ, die Datenbank und ein paar Optionen — den Rest erledigt das Playbook.
...
|------------------------------------|--------------------------|-------------|
| `k3s` / `k3s_v1_34` / `k3s_v1_35` | Kubernetes über K3s      | ✅ getestet |
| `minikube` / `minikube_v1_38`      | Kubernetes über Minikube | ✅ getestet |
...
|-------------------------|--------------|-----------------------------------------------------------------------|
| `app_name`              | Pflichtfeld  | Zu deployende Anwendung                                               |
| `app_deploy_type`       | Pflichtfeld  | Deployment-Typ (siehe oben)                                           |
| `app_db_type`           | Pflichtfeld  | Datenbank: `mariadb`, `postgresql`, `mysql`, `none`                   |
| `app_port`              | Pflichtfeld  | Interner Port (z.B. 8055=Directus, 8069=Odoo, 11434=Ollama)          |
...
| `isolation_app`         | 🚧 in Arbeit | DNS-Isolation — App kann keine externen Domains auflösen              |
...
- Ubuntu 24.04
- AWX 24.6.1 auf Minikube WSL/Win10
...
- Alle IPs, Domains und E-Mail-Adressen sind durch Platzhalter ersetzt (z.B. `<ip_vpn_server>`) — können per Suchen & Ersetzen leicht angepasst werden
